...
## operation
mix_p_zero <- mix_prob_drink  + (1 - mix_prob_drink)* mix_rate^mix_zeros * exp(- mix_rate)

mix_p_one <- (1 - mix_prob_drink) * mix_rate^mix_ones * exp(- mix_rate)

mix_p_two <- (1 - mix_prob_drink) * mix_rate^mix_twos * exp(- mix_rate) / factorial(mix_twos)

mix_p_three <- (1 - mix_prob_drink) * mix_rate^mix_threes * exp(- mix_rate) / factorial(mix_threes)

mix_p_four <- (1 - mix_prob_drink) * mix_rate^mix_fours * exp(- mix_rate) / factorial(mix_fours)

mix_p_six<- (1 - mix_prob_drink) * mix_rate^mix_sixs * exp(- mix_rate) / factorial(mix_sixs)

#combine into a matrix
mix_prob <- cbind(mix_p_zero, mix_p_one, mix_p_two, mix_p_three, mix_p_four, mix_p_six)

## distribution
distribution(mix_y_factor) <- categorical(mix_prob)

##model
mix_model <- model(log_rate, logit_drink)

##draws
mix_draws <- greta::mcmc(mix_model, n_samples = 3000)

...
##data
trolley_response_1 <- model.matrix(~ response_factor -1, trolley)
trolley_response_ones <- trolley_response_1[,1]
trolley_response_twos <- trolley_response_1[,2]
trolley_response_threes <- trolley_response_1[,3]
trolley_response_fours <- trolley_response_1[,4]
trolley_response_fives <- trolley_response_1[,5]
trolley_response_sixs <- trolley_response_1[,6]
trolley_response_sevens <- trolley_response_1[,7]

trolley_action <- trolley$action
trolley_intention <- trolley$intention
trolley_contact <- trolley$contact

##prior
trolley_bA <- normal(0, 10)
trolley_bI <- normal(0, 10)
trolley_bC <- normal(0, 10)

trolley_c1 <- normal(0, 10)
trolley_c2 <- normal(0, 10)
trolley_c3 <- normal(0, 10)
trolley_c4 <- normal(0, 10)
trolley_c5 <- normal(0, 10)
trolley_c6 <- normal(0, 10)

theta_i <- trolley_bA * trolley_action + trolley_bI * trolley_intention + trolley_bC * trolley_contact

## Operation
trolley_logit_p1 <- trolley_c1 - theta_i

trolley_logit_p2 <- trolley_c2 - theta_i

trolley_logit_p3 <- trolley_c3 - theta_i

trolley_logit_p4 <- trolley_c4 - theta_i

trolley_logit_p5 <- trolley_c5 - theta_i

trolley_logit_p6 <- trolley_c6 - theta_i

trolley_p1 <- psych::logistic(trolley_logit_p1)

trolley_p2 <- psych::logistic(trolley_logit_p2) - psych::logistic(trolley_logit_p1)

trolley_p3 <- psych::logistic(trolley_logit_p3) - psych::logistic(trolley_logit_p2)

trolley_p4 <- psych::logistic(trolley_logit_p4) - psych::logistic(trolley_logit_p3)

trolley_p5 <- psych::logistic(trolley_logit_p5) - psych::logistic(trolley_logit_p4)

trolley_p6 <- psych::logistic(trolley_logit_p6) - psych::logistic(trolley_logit_p5)

trolley_p7 <- 1 - trolley_p1 - trolley_p2 - trolley_p3 - trolley_p4 - trolley_p5 - trolley_p6


trolley_prob <- cbind(trolley_p1, trolley_p2, trolley_p3, trolley_p4, trolley_p5, trolley_p6, trolley_p7)

##distribution

distribution(trolley_response_1) <- categorical(trolley_prob)

##model
trolley_model_1 <- model(trolley_bA, trolley_bC, trolley_bI, trolley_c1, trolley_c2, trolley_c3, trolley_c4, trolley_c5, trolley_c6)

inits <- replicate(4, 
                   initials(trolley_c1 = -3, trolley_c2 = -2.5, trolley_c3 = -2, trolley_c4 = -1.5, trolley_c5 = -1, trolley_c6 = -0.5, trolley_bA = -1, trolley_bI = -1, trolley_bC = -1),
                   simplify = FALSE)

##draws
trolley_draws_1 <- greta::mcmc(trolley_model_1, n_samples = 3000, initial_values = inits)

summary(trolley_draws_1)

```


## Chapter 12.2 Zero-inflated outcomes
```{r}
#data
# define parameters
...
mix_y_factor <- model.matrix(~ y_factor - 1, mix_df)

mix_df_zeros <- mix_y_factor[,1]
mix_df_ones <- mix_y_factor[,2]
mix_df_twos <- mix_y_factor[,3]
mix_df_threes <- mix_y_factor[,4]
mix_df_fours <- mix_y_factor[,5]
mix_df_sixs <- mix_y_factor[,6]
...
mix_rate <- exp(log_rate)
mix_prob_drink <- psych::logistic(logit_drink)

#mix_drink <- binomial(length_zeros, mix_prob_drink)

## operation
#mix_p_zero <- mix_drink * 1 + (1 - mix_drink) * exp(- mix_rate)
mix_p_zero <- mix_prob_drink * 1 + (1 - mix_prob_drink) * exp(- mix_rate)

mix_p_one <- (1 - mix_prob_drink) * mix_rate * exp(- mix_rate)

mix_p_two <- (1 - mix_prob_drink) * mix_rate^2 * exp(- mix_rate) / factorial(2)

mix_p_three <- (1 - mix_prob_drink) * mix_rate^3 * exp(- mix_rate) / factorial(3)

mix_p_four <- (1 - mix_prob_drink) * mix_rate^4 * exp(- mix_rate) / factorial(4)

mix_p_six<- (1 - mix_prob_drink) * mix_rate^6 * exp(- mix_rate) / factorial(6)

distribution(mix_df_zeros) <- bernoulli(mix_p_zero)
distribution(mix_df_ones) <- bernoulli(mix_p_one)
distribution(mix_df_twos) <- bernoulli(mix_p_two)
distribution(mix_df_threes) <- bernoulli(mix_p_three)
distribution(mix_df_fours) <- bernoulli(mix_p_four)
distribution(mix_df_sixs) <- bernoulli(mix_p_six)

mix_model <- model(log_rate, logit_drink)

mix_draws <- greta::mcmc(mix_model, n_samples = 1000)
...
chimp_cov_actor_intercept_slope_alpha <- normal(0, 1, dim = c(7, 3))
...
chimp_cov_block_intercept_slope_alpha <- normal(0, 1, dim = c(6,3))
...
#draws
chimp_cov_draws <- greta::mcmc(chimp_cov_model, n_samples = 5000)

summary(chimp_cov_draws)
mcmc_trace(chimp_cov_draws, pars = "chimp_cov_sigma_a_actor")
